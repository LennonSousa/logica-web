...
const EditNote: NextPage = () => {
    ...
    const [userCanViewThis, setUserCanViewThis] = useState(true);
    ...
    useEffect(() => {
        ...
        if (user && note) {
            api.get(`notes/${note}`).then(res => {
                const noteRes: Note = res.data;

                const userNoteShare = noteRes.shares.find(item => { return item.user.id === user.id });

                if (!userNoteShare) {
                    setUserCanViewThis(false);

                    return;
                }

                setUserNoteShare(userNoteShare);
    ...
    return (
        <>
            <NextSeo
                title="Anotação"
                description="Anotação da plataforma de gerenciamento da Lógica Renováveis."
                openGraph={{
                    ...
                    images: [
                        {
                            ...
                            alt: 'Anotação | Plataforma Lógica Renováveis',
                        },
                        { url: 'https://app.logicarenovaveis.com/assets/images/logo-logica.jpg' },
                    ],
                }}
